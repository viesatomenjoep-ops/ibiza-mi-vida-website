      ...
      <section className="relative flex min-h-[65vh] flex-col justify-end overflow-hidden" aria-label={`${club.name} hero`}>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-12 pt-32 md:px-8">
          <div className="flex flex-col gap-4">
            {club.music_genre && (
              <span className="inline-flex w-fit items-center gap-2 rounded-full border border-teal/50 bg-teal/10 px-4 py-1 font-sans text-xs font-semibold uppercase tracking-widest text-teal">
                <Music size={12} />
                {club.music_genre}
              </span>
            )}
            <h1 className="font-serif text-5xl font-light text-soft-white md:text-6xl lg:text-7xl">
              {club.name}
            </h1>
            {club.tagline && (
              <p className="max-w-xl font-sans text-lg text-soft-white/70">{club.tagline}</p>
            )}
            <div className="flex flex-wrap gap-4 text-soft-white/50">
              {club.location && (
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 md:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 flex flex-col gap-10">
            ...
            {club.description && (
              <AnimatedSection>
                <h2 className="mb-4 font-serif text-3xl font-light text-midnight">
                  About {club.name}
                </h2>
                <div className="prose prose-midnight max-w-none font-sans text-base leading-relaxed text-midnight/70">
            ...
            <AnimatedSection>
              <div className="flex items-center justify-between gap-4 mb-6">
                <h2 className="font-serif text-3xl font-light text-midnight">
                  Upcoming Events
                </h2>
                <span className="rounded-full bg-sandstone px-3 py-1 font-sans text-xs font-semibold text-midnight/60">
              ...
              {events.length > 0 ? (
                ...
                <div className="rounded-2xl border-2 border-dashed border-midnight/10 p-10 text-center">
                  <p className="font-serif text-2xl font-light text-midnight/40">No events scheduled yet</p>
                  <p className="mt-2 font-sans text-sm text-midnight/30">
          ...
          <aside className="lg:col-span-1">
            <AnimatedSection className="sticky top-24">
              <div className="rounded-2xl border border-midnight/10 bg-white p-6 shadow-sm">
                <h3 className="mb-2 font-serif text-2xl font-light text-midnight">
                  Book Tickets
                </h3>
                <p className="mb-6 font-sans text-sm leading-relaxed text-midnight/60">
                ...
                {club.booking_type === 'promoter_link' ? (
                  <a
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-full bg-midnight px-6 py-4 font-sans font-semibold text-white transition-colors hover:bg-midnight/80"
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex w-full items-center justify-center gap-2 rounded-full bg-teal px-6 py-4 font-sans font-semibold text-white transition-colors hover:bg-teal-dark"
                ...
                <div className="mt-6 border-t border-midnight/10 pt-6 flex flex-col gap-2 text-center">
                  <p className="font-sans text-xs text-midnight/40">Instant reply · No booking fee · Secure</p>
                </div>
              </div>
            </AnimatedSection>
          </aside>
