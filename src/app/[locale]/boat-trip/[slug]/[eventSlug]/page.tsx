...
export async function generateMetadata({ params }: { params: { slug: string; eventSlug: string; locale: string } }): Promise<Metadata> {
  ...
  if (!ev || !venue) return staticMetadata(params.locale, 'boat-trip')
  return detailMetadata(params.locale, `boat-trip/${params.slug}/${params.eventSlug}`, ev.eventName || ev.name || venue.name, {
