...
interface Props {
  params: { venueSlug: string; eventSlug: string }
}
...
export default async function EventDetailPage({ params }: Props) {
  const { venueSlug, eventSlug } = params
  
  // Find venue
  const venues = await getVenues('en')
  const venueRef = venues.find(v => v.slug === venueSlug)
  if (!venueRef) notFound()
  
  const club = await getVenue(venueRef.id, 'en')
  if (!club) notFound()

  // Find event dates
  const allDatesGlobal = await getAllDates()
  const eventDates = allDatesGlobal.filter(d => d.venueSlug === venueSlug && d.eventSlug === eventSlug)
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-12 pt-32 md:px-8">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-4 text-ibiza-sand/80 font-bold">
              <Link href={`/club-tickets/${venueSlug}`} className="flex items-center gap-1.5 hover:text-white transition-colors">
