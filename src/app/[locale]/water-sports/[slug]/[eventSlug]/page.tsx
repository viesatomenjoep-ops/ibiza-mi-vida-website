...
export async function generateMetadata({ params }: { params: { slug: string; eventSlug: string; locale: string } }): Promise<Metadata> {
  ...
  if (!ev || !venue) return staticMetadata(params.locale, 'water-sports')
  return detailMetadata(params.locale, `water-sports/${params.slug}/${params.eventSlug}`, ev.eventName || ev.name || venue.name, {
