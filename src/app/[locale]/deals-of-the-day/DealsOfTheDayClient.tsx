  ...
  return (
    <div className="theme-monaco-vip bg-[var(--color-paper)] text-[var(--color-ink)] min-h-screen pt-8 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Filter and Date Selector Slider */}
        <div className="mb-12">
            ...
            {datePills.length > 0 && (
              <div>
                <div className="text-xs font-bold tracking-widest uppercase text-white/40 mb-4">
                  {locale === 'nl' ? 'Kies een datum' : 'Select a Date'}
                </div>
                
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10 scroll-smooth items-center">
                  
                  {/* All upcoming option */}
                  <button 
                    onClick={() => setSelectedDate('all')}
                    className={`flex flex-col items-center justify-center shrink-0 w-16 h-18 rounded-2xl border transition-all ${
                      selectedDate === 'all'
                        ? 'bg-ibiza-green border-ibiza-green text-velvet-obsidian font-bold shadow-md scale-105'
                        : 'bg-white/5 border-white/10 text-white/70 hover:bg-white/10'
                    }`}
                  >
                    <span className="text-[10px] uppercase font-bold tracking-wider">All</span>
                    <span className="text-lg font-black mt-1">🗓️</span>
                  </button>
                  ...
                  {datePills.map(p => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-[9px] uppercase font-bold tracking-wider block opacity-70">
                          {p.dayName}
                        </span>
                        <span className="text-lg font-black mt-0.5 block leading-tight">
                          {p.dayNum}
                        </span>
                        <span className="text-[8px] uppercase font-bold opacity-60 mt-0.5 block">
                          {p.monthName}
                        </span>
                      </button>
                    );
