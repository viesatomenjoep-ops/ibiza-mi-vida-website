...
// Full-width, collapsible Spotify player — lives inside the fullscreen menu overlay,
// between the logo and the first category.
export function SpotifyButton() {
  const [open, setOpen] = useState(false)

  return (
    <div className="fs-spotify">
      <button
        type="button"
        className="fs-spotify-head"
        aria-expanded={open}
        onClick={() => setOpen(o => !o)}
      >
        <span className="fs-spotify-logo">
          ...
          <span className="fs-spotify-label">Ibiza mi Vida · Playlist</span>
        </span>
        <ChevronDown size={20} className="fs-spotify-chev" style={{ transform: open ? 'rotate(180deg)' : 'none' }} />
      </button>
