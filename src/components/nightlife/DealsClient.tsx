...
export default function DealsClient({ dates, translations }: DealsClientProps) {
  ...
  return (
    <>
      <section className="pt-[calc(var(--nav-h)+16px)] pb-4 relative z-10 flex flex-col items-center text-center">
                ...
                <Link href={ctLink(date.affLink, localePath)} key={date.id || idx} className="lcard in" target="_blank" rel="noopener noreferrer">
                  <div className="media">
                    ...
                    {date.eventCover || date.venueCover ? (
                      <img src={optImg(date.eventCover || date.venueCover, 500)} loading="lazy" alt={date.eventName || date.name} style={{width:'100%', height:'100%', objectFit:'cover'}} />
                    ) : (
                      <div className="ph">
