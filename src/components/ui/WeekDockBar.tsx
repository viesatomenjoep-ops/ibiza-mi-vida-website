  ...
  const renderTile = (ds: string, i: number) => {
    ...
    return (
      ...
      >
        ...
        {variant === 'photo' && photoBg && <img src={optImg(photoBg, 192)} loading="lazy" decoding="async" alt={`Ibiza events ${format(d, 'EEEE d MMMM', { locale: L })}`} aria-hidden="true" role="presentation" className={`absolute inset-0 h-full w-full object-cover ${photoDim ? 'scale-110 blur-[2px]' : ''}`} />}
