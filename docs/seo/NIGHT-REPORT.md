...
| 3 | **`GOOGLE_PLACES_API_KEY` + `GOOGLE_PLACE_ID`** in Vercel. Zonder die twee rendert er nérgens een reviewcijfer of reviewtekst — niet in de header, niet in de hero, niet in de footer, niet op `/about-us`, en geen `Review`/`AggregateRating`-schema. "De reviews staan er niet" is bijna altijd dit. De `GOOGLE_PLACE_ID` is de `ChIJ…`-vorm uit de Place ID Finder, níét het `0x…:0x…`-nummer of de CID uit een Maps-URL. | Vercel |
