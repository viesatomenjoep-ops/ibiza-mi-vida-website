...
6. ✅ `/package-deals` — sectie per club over tafels (`ClubVipTables.tsx` voor Ushuaïa, Hï, UNVRS, Chinois, Eden, O Beach)
7. ✅ `/beach-clubs` — QuickFacts vergelijkingstabel + uitgebreid FAQ-schema in 5 talen
8. ✅ `/ferry-formentera` — prijssectie, gemeten uit de agenda + QuickFacts
9. ✅ `/club-tickets/[slug]` — redactionele blok per club via `club-history.ts` (14 clubs in 5 talen)
